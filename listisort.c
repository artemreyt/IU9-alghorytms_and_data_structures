...
int     main()
{
    ...
    /*void    insertion_sort(list_t *list)
     {
     elem_t *current = list->head->next;
     int i = 1;
     while (i++ < list->size)                      //(current != list->head)
     {
     if (current->value < current->prev->value)
     {
     elem_t *find = current;
     while (find->prev != list->head->prev && find->prev->value > current->value)
     find = find->prev;
     elem_t  *find_prev = find->prev,
     *current_prev = current->prev,
     *current_next = current->next;
     find->prev->next = current;
